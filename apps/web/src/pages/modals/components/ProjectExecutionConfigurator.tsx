...
const TEMPLATE_REQUIRED_ROLES: Record<string, string[]> = {
...
const TEMPLATE_INPUT_GUIDES: Record<string, StageInputGuide> = {
  ...
  visual_design: {
    ...
    nameLabel: '设计简报',
    typeLabel: '设计稿/原型',
    contentPlaceholder: '补充品牌风格、页面结构、交互重点、参考链接（可选）。',
  },
  tech_design: {
    name: 'technical_spec',
    type: 'document',
  ...
  code_dev: {
    ...
    nameLabel: '研发实现范围',
    typeLabel: '代码仓库',
    contentPlaceholder: '补充仓库地址、分支策略、实现范围、完成定义（可选）。',
  },
  qa_acceptance: {
    name: 'qa_test_scope',
    type: 'document',
    nameLabel: '验收范围',
    typeLabel: '文档说明',
    contentPlaceholder: '补充测试范围、验收清单、回归策略和交付标准（可选）。',
  },
              ...
              >
                ...
                {option.key !== 'none' ? (
                  <p className="text-[11px] text-slate-500 mt-1">
                    关键角色: {(TEMPLATE_REQUIRED_ROLES[option.key] || []).map((roleId) => ROLE_LABELS[roleId] || roleId).join('、')}
                  </p>
                ) : null}
              </button>
            );
