...
const TEMPLATE_LIBRARY: Record<DeliverableTemplateKind, DeliverableTemplate> = {
  ...
  design_review: {
    ...
    sectionBlueprint: [
      "- 每个设计结论必须能追溯到目标、场景或约束。",
      "- 必须明确状态反馈与响应式策略，避免只给静态视觉描述。",
      "- 审查结论必须包含可执行整改项与开发交接边界。"
    ],
    acceptanceChecklist: [
      "设计说明可直接支撑开发实现，不依赖口头补充。",
      "状态矩阵、断点策略、可访问性均具备可验证条目。",
      "审查结论明确（通过/驳回）且有整改责任项。"
    ],
    authoringRules: [
      "避免泛化视觉词，必须绑定业务场景与目标指标。",
      "优先给可实现证据，不只给抽象审美描述。"
    ],
    standardsProfile: [
      {
  ...
  visual_mockup: {
    ...
    sectionBlueprint: [
      ...
      "- 必须定义空态、错误态、加载态，避免只展示 happy path。"
    ],
    acceptanceChecklist: [
