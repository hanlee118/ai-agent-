...
- `POST /api/projects/:id/stages/submit` 在 `finalizeApproval=true` 时，不能只依据单份交付物直接进入待审批。
- 仅当当前阶段核心交付物全集齐备并通过模板校验，才允许：
  - `pendingApproval=true`
  - 当前阶段任务统一置 `done`
  - 阶段进度进入可审批状态
- 若核心交付物不齐，系统必须自动降级为“持续补充中”，并返回可解释提示，不得制造“阶段已完成”假象。

验收标准：
- 在 DESIGN 阶段只提交“设计审查卡”时，项目不得进入待审批，任务不得全部变为 `done`。
- 补齐核心交付物后再次提交，才可进入待审批状态。

### FR-018 DEV 真实实现证据门禁（重要）

- DEV 阶段 `demo_prototype` / `implementation_word` 类交付物，必须满足真实实现证据：
  - 多页面/路由信号
  - 至少 2 个 API 接口证据
  - 持久化存储设计（数据库/Schema/迁移）
  - 运行与联调说明（命令/环境变量/验证步骤）
  - 至少 2 个真实代码文件路径证据（例如 `apps/.../*.ts`）
  - 联调/验证结论证据（例如 `/health` 检查、HTTP 200、回归结论）
- 对跨境爆品场景，必须包含平台来源（TikTok/Amazon/Temu）与更新机制（轮询/Webhook/增量同步）。
- 含“仅静态/单页/mock 数据”等信号且缺少数据链路时，必须拦截，不得视为可验收。

验收标准：
- DEV 交付内容仅为静态页面说明时，模板校验和验收就绪判断必须失败。
- DEV 交付补齐接口/存储/联调证据后，方可进入可验收状态。

### FR-019 演示站点链接可达性（重要）

- `/api/projects/:id/official-site` 必须返回可直接访问的绝对 URL，不得只返回相对路径。
- 最终验收报告中的“演示站点链接”摘要，必须优先展示 `访问地址: {url}`，避免误导为本地文件路径。
- 前端“打开链接/复制链接”必须统一解析并使用可访问绝对 URL。
- 官方页产物属性必须明确为“静态交付物汇总页”，不得误导为“真实研发部署站点”。

验收标准：
- 验收报告点击“打开链接”可在浏览器直接访问。
- 复制的链接可在新标签页独立打开，不依赖当前页面路径上下文。
- 验收报告文案需包含“静态交付物汇总页”提示。

### FR-021 模板骨架占位拦截（核心）

- 任何阶段交付物只要包含以下占位语句，必须直接拦截：
  - `模板章节骨架（自动补齐）`
  - `模板章节骨架（请按模板补全）`
  - `请结合 Agent 输出正文与任务证据补全本节`
- 自动推进与回填流程不得再注入上述占位语句作为“可提交内容”。

验收标准：
- 阶段提交接口返回 `422`，并给出“模板骨架占位语句”相关错误。
- 含占位语句的交付物不得进入待审批或已批准状态。
