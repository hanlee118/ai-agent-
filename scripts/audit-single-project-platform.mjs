...
const PROJECT_ID = String(process.env.PROJECT_ID || "OCC-20260424-001").trim();
...
const checks = [
  ...
  { menu: "项目组合(projects)", key: "project.detail", method: "GET", path: `/api/projects/${encodeURIComponent(PROJECT_ID)}` },
  { menu: "项目组合(projects)", key: "project.executions", method: "GET", path: `/api/projects/${encodeURIComponent(PROJECT_ID)}/executions?limit=50` },
  { menu: "项目组合(projects)", key: "project.lifecycleAudit", method: "GET", path: `/api/projects/${encodeURIComponent(PROJECT_ID)}/lifecycle-quality-audit` },
  { menu: "项目组合(projects)", key: "project.acceptance", method: "GET", path: `/api/projects/${encodeURIComponent(PROJECT_ID)}/acceptance-report` },
  { menu: "项目组合(projects)", key: "project.artifacts", method: "GET", path: `/api/projects/${encodeURIComponent(PROJECT_ID)}/final-artifacts` },
  ...
  { menu: "知识库(knowledge-hub)", key: "knowledge.status", method: "GET", path: `/api/v1/knowledge/status?projectId=${encodeURIComponent(PROJECT_ID)}&forceRefresh=true` },
  { menu: "知识库(knowledge-hub)", key: "knowledge.list", method: "GET", path: `/api/v1/knowledge?projectId=${encodeURIComponent(PROJECT_ID)}&limit=20` },
  { menu: "知识库(knowledge-hub)", key: "knowledge.forHermes", method: "GET", path: `/api/v1/knowledge/for-hermes?projectId=${encodeURIComponent(PROJECT_ID)}&limit=20` },
  { menu: "知识库(knowledge-hub)", key: "skills.forHermes", method: "GET", path: `/api/v1/skills/for-hermes?projectId=${encodeURIComponent(PROJECT_ID)}&limit=20` },
...
function containsPlaceholderTokens(value) {
  ...
  return /(待补充|占位(词|符)?|placeholder|(?:^|[\s:：\-\[\(])(?:TODO|TBD)(?=$|[\s:：\]\),.!?])|lorem ipsum|\bxxx\b)/i.test(text);
}
...
function summarizeResult(key, data) {
  if (key === "projects.list" && Array.isArray(data)) {
    return { count: data.length, hasTargetProject: data.some((item) => item?.id === PROJECT_ID) };
  }
...
try {
  ...
  const gates = [
    { name: "api_all_pass", pass: totals.fail === 0, detail: `fail=${totals.fail}` },
    {
      name: "project_id_match",
      pass: String(projectDetailSummary?.id || "") === PROJECT_ID,
      detail: `actual=${projectDetailSummary?.id || "n/a"}`,
    },
    {
      name: "project_completed",
      pass: String(projectDetailSummary?.status || "").toLowerCase() === "completed",
      detail: `status=${projectDetailSummary?.status || "n/a"}`,
    },
    {
      name: "lifecycle_gate_pass",
      pass: Boolean(lifecycleSummary?.pass),
      detail: `blockingStages=${Number(lifecycleSummary?.blockingStageCount || 0)}`,
    },
    {
      name: "acceptance_gate_pass",
      pass: Boolean(acceptanceSummary?.qualityGatePass),
      detail: `blockingStages=${Number(acceptanceSummary?.blockingStageCount || 0)}`,
    },
    {
      name: "suspicious_deliverables_zero",
      pass: Number(acceptanceSummary?.suspiciousDeliverables || 0) === 0,
      detail: `count=${Number(acceptanceSummary?.suspiciousDeliverables || 0)}`,
    },
    {
      name: "placeholder_tokens_zero",
      pass: placeholderApiHits.length === 0 && deliverableDiagnostics.placeholderTokenHits.length === 0,
    ...
    },
    {
      ...
      pass: Boolean(artifactsSummary?.readyForAcceptance),
      detail: `missingRequired=${Number(artifactsSummary?.missingRequired || 0)}`,
    },
    {
      name: "hermes_execution_present",
      pass: AUDIT_REQUIRE_HERMES ? Number(executionSummary?.hermesCount || 0) > 0 : true,
      detail: `hermesCount=${Number(executionSummary?.hermesCount || 0)}`,
    },
    {
      name: "scripted_like_zero",
      pass: AUDIT_REQUIRE_NON_SCRIPTED ? Number(executionSummary?.scriptedLike || 0) === 0 : true,
      detail: `scriptedLike=${Number(executionSummary?.scriptedLike || 0)}`,
    },
  ];
  ...
  if (AUDIT_STRICT && !gateSummary.pass) {
    process.exitCode = 1;
  }
} finally {
